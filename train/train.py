...
def task_train(experiment_name, data_path=None):
    ...
    with mlflow.start_run(run_name="train") as run:
        ...
        train_data = np.loadtxt(
            os.path.join(data_path, "train_data_preprocessed.csv"), delimiter=","
        )
        val_data = np.loadtxt(
            os.path.join(data_path, "val_data_preprocessed.csv"), delimiter=","
        )
        ...
        plot_model_performance_metrics(
            y_val, y_pred, y_proba, ax=ax, **{"model": config["train"]["model"]["name"]}
        )
        ...
        mlflow.log_figure(
            fig, "{}_conf_mtrx_pr_curve.png".format(config["train"]["model"]["name"])
        )
        plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-path", dest="data_path", type=str, default=config["train"]["data_path"]
    )
