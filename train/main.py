...
def workflow(experiment_name, file_path):
    ...
    with mlflow.start_run() as active_run:
        ...
        preprocess_run = mlflow.run(
            ".",
            "preprocess",
            parameters={"file-path": file_path, "experiment-name": experiment_name},
        )
        # preprocess_run = mlflow.tracking.MlflowClient().get_run(preprocess_run.run_id)
        processed_data_path_uri = os.path.join(
            artifact_path_local, preprocess_run.run_id, "artifacts/data/processed"
        )
        ...
        train_run = mlflow.run(
            ...
            "train",
            parameters={
                "data-path": processed_data_path_uri,
                "experiment-name": experiment_name,
            },
        )
        # train_run = mlflow.tracking.MlflowClient().get_run(train_run.run_id)
        train_model_path_uri = os.path.join(
            artifact_path_local, train_run.run_id, "artifacts/fraud_detection"
        )
